...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  useEffect(() => {
    if (!animated || constantAnimated || !cardRef.current) return;
    const card = cardRef.current;
  ...
  }, [animated, constantAnimated]);

  // Constant Infinite Loop Animation
  useEffect(() => {
    if (!constantAnimated || !cardRef.current) return;
    const card = cardRef.current;
    card.classList.add('sweep-active');
